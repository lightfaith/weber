...
def show_marked(c, color='', *args, new_line=True, stdout=True):
    ...
    lines.append('%s%s%s%s%s' % (color, COLOR_BOLD, c,
                                 COLOR_NONE, ' '.join(str(arg) for arg in args)))
    if stdout:
        with loglock:
            for line in lines:
                print(line, end=('\n' if new_line else ''))
